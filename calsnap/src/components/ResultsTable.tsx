...
// Format date/time for display: "Mon, Dec 1" on first line, "9:00 - 23:59" on second (24hr, no AM/PM)
const formatDateTime = (date: string | null, startTime: string | null, endTime: string | null): { dateStr: string; timeStr: string } => {
...
interface DateTimeEditorProps {
    ...
    endTime: string | null;
    onSave: (date: string, startTime: string | null, endTime: string | null) => void;
    onCancel: () => void;
}

const DateTimeEditor: React.FC<DateTimeEditorProps> = ({ date, startTime, endTime, onSave, onCancel }) => {
    ...
    const handleSave = () => {
        if (isAllDay) {
            onSave(editDate, null, null);
        } else {
            onSave(editDate, editStart || '09:00', editEnd || null);
        }
    };

    return (
        <div className="absolute z-10 top-full left-0 mt-1 bg-white border border-gray-200 rounded-lg shadow-lg p-3 min-w-[260px]">
...
export const ResultsTable: React.FC<ResultsTableProps> = ({
    ...
    const handleDateTimeSave = (index: number, date: string, startTime: string | null, endTime: string | null) => {
    ...
    return (
        <div className="w-full space-y-4">
            <div className="flex justify-between items-center mb-4">
                ...
                <button
                    onClick={onAdd}
                    className="hidden sm:inline-flex items-center px-3 py-1.5 bg-blue-50 text-blue-600 rounded-lg hover:bg-blue-100 transition-colors text-sm font-medium"
                >
                ...
                <table className="w-full text-left border-collapse">
                    ...
                    <tbody className="divide-y divide-gray-100">
                        {events.map((event, index) => {
                            ...
                            const { dateStr, timeStr } = formatDateTime(safeEvent.date, safeEvent.startTime, safeEvent.endTime);
                                    ...
                                    <td className="px-4 py-2 relative">
                                        ...
                                        {editingDateTime === index && (
                                            <DateTimeEditor
                                                ...
                                                onSave={(d, s, e) => handleDateTimeSave(index, d, s, e)}
